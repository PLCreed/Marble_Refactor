...
TargetModel::TargetModel(MarbleModel *marbleModel, QObject *parent) : QAbstractListModel(parent),
...
QVariant TargetModel::currentLocationData(int role) const
{
    ...
    if (tracking->status() == PositionProviderStatusAvailable)
    {
        ...
        {
            case Qt::DisplayRole:
                return tr("Current Location: %1").arg(currentLocation.toString());
...
QVariant TargetModel::homeData(int role) const
{
    switch (role)
    {
        case Qt::DisplayRole:
            return tr("Home");
        case Qt::DecorationRole:
            return QIcon(QStringLiteral(":/icons/go-home.png"));
        case MarblePlacemarkModel::CoordinateRole:
        {
...
QVariant TargetModel::bookmarkData(int index, int role) const
{
    switch (role)
    {
        ...
        case MarblePlacemarkModel::CoordinateRole:
            return qVariantFromValue(m_bookmarks[index]->lookAt()->coordinates());
    }
