...
void GeoLineStringGraphicsItem::paint(GeoPainter *painter, const ViewportParams *viewport,
                                      const QString &layer, int tileLevel)
{
...
void GeoLineStringGraphicsItem::paintInline(GeoPainter *painter, const ViewportParams *viewport)
{
    ...
    if (s_paintInline)
    {
        m_renderLabel = float(painter->pen().widthF()) >= 6.0f;
                                                        ...
                                                        const bool isOutline) const
{
    ...
    else
    {
        ...
        const QColor linePaintedColor =
            (!isOutline && (lineStyle.cosmeticOutline() && lineStyle.penStyle() == Qt::SolidLine))
        ...
        if (linePhysicalWidth != 0.0f)
        {
            const float scaledLinePhysicalWidth = float(viewport->radius()) / float(EARTH_RADIUS ) * linePhysicalWidth;
        ...
        // so we carefully check whether applying the setter is needed
        if ((currentPen.widthF() != newLineWidth) && (newLineWidth != 0.0f))
        {
...
bool GeoLineStringGraphicsItem::configurePainterForLabel(GeoPainter *painter, const ViewportParams *viewport,
