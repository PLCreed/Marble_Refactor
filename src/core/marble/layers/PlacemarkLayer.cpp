            ...
            for (int i = symbolX % radius4, width = viewport->width(); i <= width; i += radius4)
            {
                ...
                if (!mark->symbolPixmap().isNull())
                {
                    ...
                    QPainter::PixmapFragment pixmapFragment =
                        QPainter::PixmapFragment::create(QPointF(symbolPos + symbolRect.center()), QRectF(symbolRect));
...
#ifdef BATCH_RENDERING
                QRect symbolRect = mark->symbolPixmap().rect();
                QPainter::PixmapFragment pixmapFragment = QPainter::PixmapFragment::create(QPointF(symbolPos + symbolRect.center()), QRectF(
                                                                                               symbolRect));
