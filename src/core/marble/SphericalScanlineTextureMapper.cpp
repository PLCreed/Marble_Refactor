...
SphericalScanlineTextureMapper::RenderJob::RenderJob(StackedTileLoader *tileLoader, int tileLevel,
...
void SphericalScanlineTextureMapper::mapTexture(GeoPainter *painter, const ViewportParams *viewport,
...
void SphericalScanlineTextureMapper::mapTexture(const ViewportParams *viewport, int tileZoomLevel,
                                                MapQuality mapQuality)
{
    ...
    const int yTop = (imageHeight / 2 - radius >= 0) ? int(imageHeight / 2 - radius) : 0;
    const int yBottom = (yTop == 0) ? imageHeight - skip : int(yTop + radius + radius - skip);
    ...
    for (int i = 0; i < numThreads; ++i)
    {
        ...
        QRunnable *const job = new RenderJob(m_tileLoader, tileZoomLevel, &m_canvasImage,
                                             viewport, mapQuality, yStart, yEnd);
...
void SphericalScanlineTextureMapper::RenderJob::run()
{
    ...
    for (int y = m_yTop; y < m_yBottom; ++y)
    {
        ...
        const int xIpRight = (imageWidth / 2 - rx > 0) ?
                             n * int(xRight / n - 1) : n * int(xRight / n - 1) + 1;
