...
void MarbleClockPrivate::timerTimeout()
{
    ...
    qint64 msecdelta = m_lasttime.msecsTo(curenttime);
    ...
    int sleeptime = int((m_updateInterval * 1000.0 - qreal(m_datetime.time().msec()
                                                           + m_datetime.time().second() * 1000)) / m_speed);
    if (sleeptime < 1000)
    {
...
MarbleClock::MarbleClock(QObject *parent) : QObject(parent),
    d(new MarbleClockPrivate(this))
{
