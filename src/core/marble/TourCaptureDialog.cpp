...
TourCaptureDialog::TourCaptureDialog(MarbleWidget *widget, QWidget *parent) : QDialog(parent),
...
void TourCaptureDialog::loadDestinationFile()
{
    ...
    {
        filter += QLatin1String(";;") + formats.at(i).name() + QLatin1String(" (*.") +
                  formats.at(i).extension() + QLatin1Char(')');
    }
...
{
    ui->progressBar->setValue(int(position));
}
...
void TourCaptureDialog::recordNextFrame()
{
    ...
    else
    {
        ...
        ui->progressBar->setValue(int(duration * 100));
        ui->startButton->setText(tr("Start"));
...
{
    m_playback = playback;
    ui->progressBar->setMaximum(int(playback->duration() * 100));
}
