...
bool GeoPainterPrivate::doClip(const ViewportParams *viewport)
{
    ...
    const qint64 radius = qint64(viewport->radius() * viewport->currentProjection()->clippingRadius());
...
{
    angle = qreal(fmodf(float(angle), 360));
                             ...
                             qreal width, qreal height, bool isGeoProjected)
{
    ...
    else
    {
        ...
        const int precision = int(qMin<qreal>(width / degreeResolution / 8 + 1, 81));
...
QRegion GeoPainter::regionFromRect(const GeoDataCoordinates &centerCoordinates,
                                   qreal width, qreal height,
                                   bool isGeoProjected, qreal strokeWidth) const
{
    if (!isGeoProjected)
    {
        int pointRepeatNum;
...
void GeoPainter::drawRoundedRect(const GeoDataCoordinates &centerPosition,
                                 qreal width, qreal height, qreal xRnd, qreal yRnd)
{
    int pointRepeatNum;
    qreal y;
    bool globeHidesPoint;
...
void GeoPainter::drawTextFragment(const QPoint &position, const QString &text,
                                  const qreal fontSize, const QColor &color, const Frames &flags)
{
