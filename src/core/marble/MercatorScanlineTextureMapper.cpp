...
MercatorScanlineTextureMapper::RenderJob::RenderJob(StackedTileLoader *tileLoader, int tileLevel,
...
void MercatorScanlineTextureMapper::mapTexture(GeoPainter *painter, const ViewportParams *viewport,
...
void MercatorScanlineTextureMapper::mapTexture(const ViewportParams *viewport, int tileZoomLevel, MapQuality mapQuality)
{
    ...
    const int yTop  = int(qBound(qreal(0.0), realYTop, qreal(imageHeight)));
    int yPaintedTop = yTop;
    int yPaintedBottom = int(qBound(qreal(0.0), realYBottom, qreal(imageHeight)));
...
void MercatorScanlineTextureMapper::RenderJob::run()
{
    ...
    const qreal pixel2Rad = 1.0 / qreal(rad2Pixel);
    ...
    const int yCenterOffset = int(asinh(tan(centerLat)) * qreal(rad2Pixel));
