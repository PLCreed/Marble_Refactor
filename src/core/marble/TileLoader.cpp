...
QImage TileLoader::loadTileImage(GeoSceneTextureTileDataset const *textureLayer, TileId const &tileId,
...
GeoDataDocument *TileLoader::loadTileVectorData(GeoSceneVectorTileDataset const *textureLayer,
                                                TileId const &tileId, DownloadUsage const usage)
{
    // FIXME: textureLayer->fileFormat() could be used in the future for use just that parser,
    //        instead of all available parsers
...
bool TileLoader::baseTilesAvailable(GeoSceneTileDataset const &tileData)
{
    ...
    for (int column = 0; result && column < levelZeroColumns; ++column)
    {
        for (int row = 0; result && row < levelZeroRows; ++row)
        {
            ...
            if (!result)
            {
                mDebug() << "Base tile " << tileData.relativeTileFileName(id) << " is missing for source dir "
                         << tileData.sourceDir();
            }
        }
    }
...
void TileLoader::triggerDownload(GeoSceneTileDataset const *tileData, TileId const &id, DownloadUsage const usage)
{
    if (id.zoomLevel() > 0)
    {
        int minValue = tileData->maximumTileLevel() == -1 ?
                       id.zoomLevel() : qMin(id.zoomLevel(), tileData->maximumTileLevel());
    ...
    QString const idStr = QString("%1:%2:%3:%4:%5").arg(tileData->nodeType(), tileData->sourceDir())
                          .arg(id.zoomLevel()).arg(id.x()).arg(id.y());
...
QImage TileLoader::scaledLowerLevelTile(const GeoSceneTextureTileDataset *textureData, TileId const &id)
{
    ...
    for (int level = qMax<int>(0, id.zoomLevel() - 1); level >= 0; --level)
    {
        ...
        {
            mDebug() << "No level zero tile installed in map theme dir. "
                "Falling back to a transparent image for now.";
...
GeoDataDocument *TileLoader::openVectorFile(const QString &fileName) const
{
    ...
    mDebug() << "Unable to open vector tile " << fileName
