...
class KBiAssociativeContainer
{
    ...
    {
        /* implicit */
        _iterator_impl_ctor(const typename QHash<T, U>::iterator it)
    ...
    {
        /* implicit */
        _iterator_impl_ctor(const typename QMap<T, U>::iterator it)
        // Using internals here because I was too lazy to write my own iterator.
#if QT_VERSION < 0x050000
...
private:
    ...
    friend QDataStream &
    operator<<<LeftContainer, RightContainer>(QDataStream & out, const KBiAssociativeContainer<LeftContainer, RightContainer> &bihash);
