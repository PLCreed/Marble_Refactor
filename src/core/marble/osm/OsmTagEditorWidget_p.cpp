...
QStringList OsmTagEditorWidgetPrivate::generateTagFilter() const
{
    ...
    // If the placemark has highway=*, recommend "lanes=*", "maxspeed=*",
    // "oneway=*", "service=*", "bridge=*", "tunnel=*"
    tags = QStringList() << "highway=*";
    tagsAux = QStringList() << "lanes=*" << "maxspeed=*" << "maxheight=*" << "maxweight=*" << "abutters=*"
                            << "oneway=*" << "service=*" << "bridge=*" << "tunnel=*";
    if (geodata_cast<GeoDataLineString>(m_placemark->geometry()) && containsAny(osmData, tags))
    {
        addPattern(filter, osmData, tagsAux);
    ...
    tags = QStringList() << "addr:street=*" << "addr:housenumber=*" << "addr:postcode=*"
                         << "addr:country=*" << "access=*";
    addPattern(filter, osmData, tags);
...
void OsmTagEditorWidgetPrivate::addPattern(QStringList &filter, const OsmPlacemarkData &osmData,
