    ...
    m_addPrimitiveButton(new QToolButton)
{
    ...
    m_actionAddFlyTo = new QAction(QIcon(QStringLiteral(":/marble/flag.png")),
                                   QObject::tr("Add FlyTo"), addPrimitiveMenu);
    addPrimitiveMenu->addAction(m_actionAddFlyTo);
    m_actionAddWait = new QAction(QIcon(QStringLiteral(":/marble/player-time.png")),
                                  QObject::tr("Add Wait"), addPrimitiveMenu);
    addPrimitiveMenu->addAction(m_actionAddWait);
    m_actionAddSoundCue = new QAction(QIcon(QStringLiteral(":/marble/audio-x-generic.png")),
                                      QObject::tr("Add SoundCue"), addPrimitiveMenu);
    addPrimitiveMenu->addAction(m_actionAddSoundCue);
    addPrimitiveMenu->addSeparator();
    m_actionAddPlacemark = new QAction(QIcon(QStringLiteral(":/icons/add-placemark.png")),
                                       QObject::tr("Add Placemark"), addPrimitiveMenu);
    addPrimitiveMenu->addAction(m_actionAddPlacemark);
    m_actionAddRemovePlacemark = new QAction(QIcon(QStringLiteral(":/icons/remove.png")),
                                             QObject::tr("Remove placemark"), addPrimitiveMenu);
    addPrimitiveMenu->addAction(m_actionAddRemovePlacemark);
    m_actionAddChangePlacemark = new QAction(QIcon(QStringLiteral(":/marble/document-edit.png")),
                                             QObject::tr("Change placemark"), addPrimitiveMenu);
...
void TourWidget::closeEvent(QCloseEvent *event)
{
    ...
    const int result = QMessageBox::question(d->m_widget, QObject::tr("Save tour"),
...
void TourWidget::handleSliderMove(int value)
{
    ...
    QTime time = nullTime.addSecs(int(value / 100.0));
    d->m_tourUi.m_elapsedTime->setText(QString("%L1:%L2")
...
void TourWidgetPrivate::openFile()
{
    ...
    {
        QString const filename = QFileDialog::getOpenFileName(q, QObject::tr("Open Tour"), QDir::homePath(),
                                                              QObject::tr("KML Tours (*.kml)"));
        if (!filename.isEmpty())
        {
...
void TourWidgetPrivate::deleteSelected()
{
    ...
    QPointer<QMessageBox> dialog = new QMessageBox(QMessageBox::Question, title, text,
...
void TourWidgetPrivate::updateButtonsStates()
{
    ...
    else
    {
        ...
        if (GeoDataPlaylist *playlist = (rootObject ? geodata_cast<GeoDataPlaylist>(rootObject) : nullptr))
        {
            // if we can move down enable action else disable.
            m_tourUi.m_actionMoveDown->setEnabled((end->row() != playlist->size() - 1));
        }
    }
}
...
void TourWidgetPrivate::updateRootIndex()
{
    ...
    if (tour)
    {
        ...
        if (playlist)
        {
            ...
            QObject::connect(m_tourUi.m_listView->selectionModel(),
                             SIGNAL(selectionChanged(QItemSelection,QItemSelection)),
                             q, SLOT(updateButtonsStates()));
        }
        m_playback.setMarbleWidget(m_widget);
        m_playback.setTour(tour);
        m_tourUi.m_slider->setMaximum(int(m_playback.duration() * 100));
        QTime nullTime(0, 0, 0);
        QTime time = nullTime.addSecs(int(m_playback.duration()));
        m_tourUi.m_totalTime->setText(QString("%L1:%L2")
                                      .arg(time.minute(), 2, 10, QLatin1Char('0'))
                                      .arg(time.second(), 2, 10, QLatin1Char('0')));
...
{
    d->m_tourUi.m_slider->setMaximum(int(d->m_playback.duration() * 100));
    QTime nullTime(0, 0, 0);
    QTime totalTime = nullTime.addSecs(int(d->m_playback.duration()));
    d->m_tourUi.m_totalTime->setText(QString("%L1:%L2")
                                     .arg(totalTime.minute(), 2, 10, QLatin1Char('0'))
                                     .arg(totalTime.second(), 2, 10, QLatin1Char('0')));
...
void TourWidgetPrivate::saveTourAs()
{
    if (m_document)
    {
        QString const filename = QFileDialog::getSaveFileName(q, QObject::tr("Save Tour as"),
                                                              QDir::homePath(), QObject::tr("KML Tours (*.kml)"));
        if (!filename.isEmpty())
        {
            saveTourAs(filename);
        }
...
void TourWidgetPrivate::handlePlaybackProgress(const double position)
{
    ...
    {
        m_tourUi.m_slider->setValue(int(position * 100));
        QTime nullTime(0, 0, 0);
        QTime time = nullTime.addSecs(int(position));
        m_tourUi.m_elapsedTime->setText(QString("%L1:%L2")
                                        .arg(time.minute(), 2, 10, QLatin1Char('0'))
                                        .arg(time.second(), 2, 10, QLatin1Char('0')));
    }
}
...
void TourWidget::setHighlightedItemIndex(int index)
{
    ...
    for (int i = 0; i < playlist->size(); i++)
    {
        ...
        GeoDataObject *object = qvariant_cast<GeoDataObject *>
                                    (currentIndex.data(MarblePlacemarkModel::ObjectPointerRole));
