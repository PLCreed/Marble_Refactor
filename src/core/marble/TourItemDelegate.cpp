...
void TourItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (!m_editingIndices.contains(index))
    {
        ...
        {
            GeoDataCoordinates const flyToCoords = index.data(MarblePlacemarkModel::CoordinateRole)
...
QWidget *TourItemDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option,
                                        const QModelIndex &index) const
{
    ...
    if (geodata_cast<GeoDataFlyTo>(object))
    {
        ...
        connect(this, SIGNAL(firstFlyToChanged(QPersistentModelIndex)),
                widget, SLOT(setFirstFlyTo(QPersistentModelIndex)));
...
bool TourItemDelegate::editAnimatedUpdate(GeoDataAnimatedUpdate *animatedUpdate, bool create)
{
    ...
    if (create && !((animatedUpdate->update()->create() == nullptr)
                    || (animatedUpdate->update()->create()->size() == 0)))
    {
    ...
    else if (!create && !((animatedUpdate->update()->change() == nullptr)
                          || (animatedUpdate->update()->change()->size() == 0)))
    {
    ...
    if (!create)
    {
        if (placemark->targetId().isEmpty() && !defaultFeatureId().isEmpty())
        {
            ...
            if (GeoDataPlacemark *targetPlacemark = ((feature != nullptr) ?
                                                     geodata_cast<GeoDataPlacemark>(feature) : nullptr))
            {
...
bool TourItemDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option,
        ...
        if (editRect.contains(mouseEvent->pos()))
        {
            ...
            else
            {
                GeoDataObject *object = qvariant_cast<GeoDataObject *>
                                            (index.data(MarblePlacemarkModel::ObjectPointerRole));
