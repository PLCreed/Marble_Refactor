...
QVariant NewstuffModel::data(const QModelIndex &index, int role) const
{
    if (index.isValid() && (index.row() >= 0) && (index.row() < d->m_items.size()))
    {
        switch (role)
        {
            ...
            case InstalledReleaseDate:
                return d->m_items.at(index.row()).installedReleaseDate();
            case InstalledFiles:
            ...
            case IsTransitioning:
                return d->isTransitioning(index.row());
            case PayloadSize:
            {
...
void NewstuffModel::contentsListed(int exitStatus)
{
    ...
    {
        mDebug() << "Process exit status " << exitStatus << " indicates an error.";
        emit installationFailed(d->m_currentAction.first,
                                QString("Unable to list file contents. Process exited with status code %1.").arg(exitStatus));
