...
public:
    RenderJob(StackedTileLoader *tileLoader, int tileLevel, QImage *canvasImage,
              const ViewportParams *viewportParams, MapQuality mapQuality, int yTop, int yBottom);
...
EquirectScanlineTextureMapper::RenderJob::RenderJob(StackedTileLoader *tileLoader, int tileLevel,
                                                    QImage *canvasImage, const ViewportParams *viewport,
                                                    MapQuality mapQuality, int yTop, int yBottom) :
...
void EquirectScanlineTextureMapper::mapTexture(GeoPainter *painter, const ViewportParams *viewport,
                                               int tileZoomLevel, const QRect &dirtyRect,
...
void EquirectScanlineTextureMapper::mapTexture(const ViewportParams *viewport, int tileZoomLevel, MapQuality mapQuality)
{
    ...
    int yCenterOffset = int(centerLat * qreal(rad2Pixel));

    // Calculate y-range the represented by the center point, yTop and
    // what actually can be painted
    const int yTop  = int(imageHeight / 2 - radius + yCenterOffset);
    int yPaintedTop = int(imageHeight / 2 - radius + yCenterOffset);
    int yPaintedBottom = int(imageHeight / 2 + radius + yCenterOffset);

    if (yPaintedTop < 0)
        yPaintedTop = 0;
    if (yPaintedTop > imageHeight)
        yPaintedTop = imageHeight;
    if (yPaintedBottom < 0)
        yPaintedBottom = 0;
    if (yPaintedBottom > imageHeight)
    ...
    for (int i = 0; i < numThreads; ++i)
    {
        ...
        QRunnable *const job = new RenderJob(m_tileLoader, tileZoomLevel, &m_canvasImage, viewport,
                                             mapQuality, yStart, yEnd);
...
void EquirectScanlineTextureMapper::RenderJob::run()
{
    ...
    const float pixel2Rad = float(1.0 / rad2Pixel);  // FIXME changing to qreal may crash Marble when the equator is visible
