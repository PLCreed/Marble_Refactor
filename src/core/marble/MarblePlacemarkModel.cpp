...
QVariant MarblePlacemarkModel::data(const QModelIndex &index, int role) const
{
    ...
    {
        return qVariantFromValue(d->m_placemarkContainer->at(index.row())->extendedData()
                                 .value(QStringLiteral("gmt")).value());
    }
    else if (role == DstRole)
    {
        return qVariantFromValue(d->m_placemarkContainer->at(index.row())->extendedData()
                                 .value(QStringLiteral("dst")).value());
    }
    ...
    {
        return qVariantFromValue(d->m_placemarkContainer->at(index.row())->coordinate()
                                 .longitude(GeoDataCoordinates::Degree));
    }
...
void MarblePlacemarkModel::removePlacemarks(const QString &containerName, int start, int length)
{
    if (length > 0)
    {
        ...
        emit layoutChanged();
        emit countChanged();
        mDebug() << "removePlacemarks(" << containerName << "): Time elapsed:" << t.elapsed()
                 << "ms for" << length << "Placemarks.";
    }
}
