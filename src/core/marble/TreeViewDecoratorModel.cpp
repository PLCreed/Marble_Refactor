...
bool TreeViewDecoratorModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
    ...
    const GeoDataObject *object = qvariant_cast<GeoDataObject *>
                                      (rowIndex.data(MarblePlacemarkModel::ObjectPointerRole));
...
QVariant TreeViewDecoratorModel::data(const QModelIndex &proxyIndex, int role) const
{
    ...
    GeoDataObject *object = qvariant_cast<GeoDataObject *>
                                (QSortFilterProxyModel::data(proxyIndex, MarblePlacemarkModel::ObjectPointerRole));
    if (!object)
    {
        return QSortFilterProxyModel::data(proxyIndex, role);
    }
