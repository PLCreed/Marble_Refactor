...
void RouteSimulationPositionProviderPlugin::updateRoute()
{
    ...
    if (canWork)
    {
        ...
        m_updateTimer.start(int(1000.0 / c_frequency));
    }
    else
    {
...
void RouteSimulationPositionProviderPlugin::update()
{
    ...
    if ((m_currentIndex >= 0) && (m_currentIndex < m_lineStringInterpolated.size()))
    {
        ...
        if (m_currentPosition.isValid())
        {
            ...
            qreal checkedDistance = m_currentPosition.sphericalDistanceTo(m_lineStringInterpolated.at(m_currentIndex)) *
                                    m_marbleModel->planetRadius();
            ...
            {
                qreal previousHeading = m_lineStringInterpolated.at(i - 1).bearing(m_lineStringInterpolated.at(
                                                                                       i), GeoDataCoordinates::Degree,
                ...
                {
                    qreal newHeading = m_lineStringInterpolated.at(j - 1).bearing(m_lineStringInterpolated.at(
                                                                                      j), GeoDataCoordinates::Degree,
                                                                                  GeoDataCoordinates::FinalBearing);
                    ...
                    curveLength += m_lineStringInterpolated.at(j - 1).sphericalDistanceTo(m_lineStringInterpolated.at(j)) *
                                   m_marbleModel->planetRadius();
                }
                checkedDistance += m_lineStringInterpolated.at(i).sphericalDistanceTo(m_lineStringInterpolated.at(i + 1)) *
                                   m_marbleModel->planetRadius();
            }
            m_speed = newSpeed;
...
GeoDataCoordinates RouteSimulationPositionProviderPlugin::addNoise(const Marble::GeoDataCoordinates &position,
                                                                   const Marble::GeoDataAccuracy &accuracy) const
{
